...
class Maze(tk.Tk):
    def __init__(self):
        ...
        self.action_map = {i: v for i, v in enumerate(self.action_space)}  # 用来转义网络输出的action值
        self.n_features = 2
        self.title("maze")
    ...
    def normalize_state_coord(self, coord):
        oval_coord = self.canvas.coords(self.oval)
        return (np.array(coord)[:self.n_features] - np.array(oval_coord)[:self.n_features]) / (MAZE_H * UNIT)
    ...
    def reset(self):
        ...
        return self.normalize_state_coord(self.canvas.coords(self.rect))
    ...
    def step(self, action):
        ...
        next_coord = self.canvas.coords(self.rect)  # next_state
        ...
        s_ = self.normalize_state_coord(next_coord)
